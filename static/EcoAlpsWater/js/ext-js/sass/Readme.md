...
folder are mapped to the application's root namespace, 'EcoAlpsWater'. This is set in
`"app.json"`:

    "sass": {
        "namespace": "EcoAlpsWater"
    }
...
included in the application's JavaScript code build. By default, files in this 
folder are mapped to the application's root namespace, 'EcoAlpsWater' in the same way
as `"EcoAlpsWater/sass/src"`.
